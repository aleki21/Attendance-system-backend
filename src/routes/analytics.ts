  ...
  try {
    ...
    const attendanceStats = await db
      .select({
        ...
        presentCount: sql<number>`count(case when ${attendance.status} = 'present' then 1 end)`.as('present_count'),
        totalCount: sql<number>`count(${attendance.attendanceId})`.as('total_count')
      })
      .from(events)
    ...
    const genderTrends = await db
      .select({
        ...
        `.as('female_count'),
        total: sql<number>`count(case when ${attendance.status} = 'present' and ${members.ageGroup} = 'adult' then 1 end)`.as('total_count')
      })
      .from(events)
      .leftJoin(attendance, eq(events.eventId, attendance.eventId))
      ...
      .where(
        and(
          gte(events.date, startDateStr),
          lte(events.date, endDateStr),
          eq(members.ageGroup, 'adult') // Explicitly filter for adults only
        )
      )
      .groupBy(events.eventId, events.name, events.date)
      .orderBy(events.date);
